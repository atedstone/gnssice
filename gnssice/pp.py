...
def remove_displacement_outliers(
    ...
    :param diff_x_m: absolute difference threshold for along-track
    :param diff_y_m: absolute difference threshold for across-track
    :param diff_z_m: absolute difference threshold for elevation
...
def calculate_epoch_sigmas(
    ...
    """
    Given observed sigma time series, calculate the sigma at a target
    ...
    ## Iterate through the time series in chunks which correspond 
    ## either exactly or roughly to the target_freq.
    ...
    while True:   
        ...
        epoch = epoch.loc[:epoch.index[0] + pd.Timedelta(avg_window)]
        if len(epoch) == 0:
            break
        ...
        store[d] = ss
        # Increment the pointer to the start of the next period
        ptr = d + pd.Timedelta(target_freq)

    if align_to_midnight:
        return (pd.Series(store), pd.Series(store_diff))
    else:
    ...
    **kwargs
    ...
    if calc_uncertainty: 
        ...
        if isinstance(sigmas, tuple):
            ...
            epoch_uncs, _ = calculate_epoch_sigmas(sigmas[0], sigmas[1], period, '3h', **kwargs)
